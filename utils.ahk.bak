...
get_screens_data(scr, debug:=0){
	; TODO: auto enumerate screens based on coordinates
	local
	...
	if (moncount>1) {
		for _, i in range(moncount-1) {
			for _, j in range(i+1, moncount) {
				if (scr["Xrange"][i][1] > scr["Xrange"][j][1]) {
					temp := scr["order"][i]
					scr["order"][i] := scr["order"][j]
					scr["order"][j] := temp
				}
			}
		}
